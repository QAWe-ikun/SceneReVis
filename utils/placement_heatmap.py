...
class TwoWayHeatmapDecoder(nn.Module):
    ...
    def forward_tokens(
        ...
        return room_tokens, prompt_tokens, (H, W)
    ...
    def forward(
        ...
        room_tokens, _, (H, W) = self.forward_tokens(
            room_features,
            obj_patches,
            text_tokens,
            text_mask,
...
class TwoWayFusionDecoder(nn.Module):
    """Two-way prompt/dense fusion without a dense heatmap output head."""
...
class HSDCoordinateHead(nn.Module):
    """Learned placement query + MLP coordinate head for the HSD-F baseline."""
    ...
    def __init__(self, hidden_dim: int = 256, num_heads: int = 8):
        ...
        self.coord_query = nn.Parameter(torch.zeros(1, 1, hidden_dim))
        nn.init.normal_(self.coord_query, std=0.02)
        ...
        self.coord_pos_proj = nn.Sequential(
    ...
    def forward(
        ...
        if room_tokens.size(1) != grid_h * grid_w:
            raise ValueError(
                f"room token count {room_tokens.size(1)} does not match grid {grid_h}x{grid_w}"
            )
        ...
        room_tokens = room_tokens + self.coord_pos_proj(pos)
        query = self.coord_query.expand(room_tokens.size(0), -1, -1)
        attended, _ = self.query_attn(
            query=query,
            key=room_tokens,
            value=room_tokens,
        )
        coord_token = self.query_norm(query + attended).squeeze(1)
        return torch.sigmoid(self.head(coord_token))
...
class PlacementHSDF(nn.Module):
    """HSD-F baseline: same encoders and two-way fusion, MLP coordinate output."""
    ...
    def __init__(
        ...
        siglip_model = resolve_siglip_model(siglip_model)
        print(f"[PlacementHSDF] Using SigLIP object/text model: {siglip_model}")
        ...
        if self.room_encoder_type == "dinov2":
            dino_model = resolve_dinov2_model(dino_model)
            print(f"[PlacementHSDF] Using DINOv2 room model: {dino_model}")
            self.room_encoder = DinoVisionEncoder(
                dino_model,
                image_size=room_image_size or DINO_IMAGE_SIZE,
            )
        ...
        self.fusion_decoder = TwoWayFusionDecoder(
            room_dim=room_dim,
            prompt_dim=siglip_dim,
            hidden_dim=hidden_dim,
            num_heads=num_heads,
        ...
        self.coord_head = HSDCoordinateHead(
            hidden_dim=hidden_dim,
            num_heads=num_heads,
        )
    ...
    def forward(
        ...
        if self.room_encoder_type == "dinov2":
            spatial_features = self.room_encoder.encode_spatial(room_image)
        else:
            spatial_features = self.vision_encoder.encode_spatial(room_image)

        if object_image is not None:
            obj_patches = self.vision_encoder.encode_patches(object_image)
        else:
            device = spatial_features.device
            obj_patches = torch.zeros(
                B,
        ...
        text_tokens, text_mask = self.text_encoder.encode(object_desc)
        room_tokens = self.fusion_decoder(
            spatial_features,
            obj_patches,
            text_tokens,
            text_mask,
        )
        return self.coord_head(room_tokens, spatial_features.shape[1:3])
